...
Reads all raw material (`.opencode/stash/*.md` + `.opencode/friction/antigen_clusters.json`), extracts durable facts, episodes, and behavioral antigens into a single `.opencode/memory/MEMORY.md`, then injects a managed memory section into `AGENTS.md`.
...
3. **Merge into MEMORY.md**
   - Read existing `.opencode/memory/MEMORY.md` and parse its sections (## Facts, ## Episodes, ## Antigens)
   ...
   - Write merged result to `.opencode/memory/MEMORY.md` in the format under step 6.
...
- Friction output: `.opencode/friction/antigen_clusters.json` (observed-reaction clusters: theme, suggested_artifact, confidence, severity, sessions, projects, contexts)
