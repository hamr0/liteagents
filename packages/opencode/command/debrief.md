...
`.opencode/remember/fix-ledger.md`, in the exact format `/branch-review` writes
(see that skill), tagged `nit` or `change` — the size of the fix, not its
severity. Dedupe first with plain `grep -F "<snippet>" .opencode/remember/fix-ledger.md`
