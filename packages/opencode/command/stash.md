...
4. **Consolidation nudge** — whichever actor wrote the file (the subagent, or the session
   ...
   ```bash
   find "$ROOT/.opencode/stash/" -maxdepth 1 -name '*.md' ! -name '.*' 2>/dev/null | wc -l   # total
   if [ -f "$ROOT/.opencode/remember/.processed" ]; then grep -c '' "$ROOT/.opencode/remember/.processed"; else echo 0; fi   # processed
   ```
