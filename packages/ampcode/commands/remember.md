...
1. **Gather sources**
   - **Legacy layout migration (one-time, loud).** Older versions used `.claude/memory/` and
     ...
     `.claude/memory/{MEMORY.md,ledger.json,.processed}` → `.amp/remember/`, and **discard**
...
8. **Report to user** — print it AND write the same content to `.amp/remember/report.md`
   ...
   - **Mechanical length check** — run, don't estimate. This confirms the step-3 gate rather
     ...
     ```bash
     awk '/^## Facts/{f=1} /^## Episodes/{f=0} f && /^- / && length($0)>182' .amp/remember/MEMORY.md
     ```
