...
Live mode needs a one-time install. Two steps:
  ...
  1. From this repo's root (or wherever liteagents lives):
     ...
     This copies the marketplace into ~/.config/amp/plugins/ and runs npm install.

  2. In any Claude session, register and install the plugin:
       /plugin marketplace add ~/.config/amp/plugins/live-canvas-marketplace
...
**One template, every framework:** `~/.config/amp/commands/live-canvas/templates/overlay-vanilla.js`. Single file, zero dependencies, plain DOM. Works in vanilla JS, Vue, Svelte, Rails, Django, Phoenix, plain HTML, Next.js, Vite-React, Remix — anywhere a `<script>` tag runs.
...
5. **Feedback Overlay** (CRITICAL - NEVER OMIT):
   ...
   - Copy `~/.config/amp/commands/live-canvas/templates/overlay-vanilla.js` into a directory served by the dev server (e.g. `public/`, `static/`, or wherever the framework serves static assets).
