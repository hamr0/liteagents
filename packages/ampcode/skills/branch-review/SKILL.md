...
- **No edits — three exceptions.** You have no authorization to change code,
  ...
  delete), `.amp/remember/last-review.md` (overwrite; the review record
...
- **Fix ledger: N nits, K changes — M added this run** (M = appended this
  ...
  .amp/remember/fix-ledger.md` = total, `grep -c '^[- ].*· change$'
  .amp/remember/fix-ledger.md` = K, N = total − K. N + K > 0 → add: "N + K
