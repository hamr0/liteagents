...
allowed-tools: Read, Grep, Glob, Edit, Agent, Bash(git diff:*), Bash(git status:*), Bash(git log:*)
...
`.claude/remember/fix-ledger.md`, in the exact format `/branch-review`
writes, tagged `nit` or `change`, using the worker's bullet text verbatim.
Dedupe first with plain `grep -F "<snippet>" .claude/remember/fix-ledger.md`
